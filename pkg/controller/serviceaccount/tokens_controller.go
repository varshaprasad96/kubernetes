...
func (e *TokensController) ensureReferencedToken(serviceAccount *v1.ServiceAccount) ( /* retry */ bool, error) {
	...
	if !addedReference {
		...
		klog.V(2).Infof("deleting secret %s|%s/%s because reference couldn't be added (%v)", secret.ClusterName, secret.Namespace, secret.Name, err)
...
func (e *TokensController) generateTokenIfNeeded(serviceAccount *v1.ServiceAccount, cachedSecret *v1.Secret) ( /* retry */ bool, error) {
	...
	if liveSecret.ResourceVersion != cachedSecret.ResourceVersion {
		...
		klog.V(2).Infof("secret %s|%s/%s is not up to date, skipping token population", liveSecret.ClusterName, liveSecret.Namespace, liveSecret.Name)
		return false, nil
	}
